...
public class ActivityTransJual_Proto extends AppCompatActivity {
    ...
    EditText kdbar, nambar, satbar, hrgbar, sup;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_trans_jual__proto);
        ...
        kdtrans.setText("TJ"+String.valueOf(randomNumber));
...
//
//                } else {
                Cursor cek = db.rawQuery("SELECT * FROM ISIBARANG WHERE KDBAR ='" + kdbar.getText().toString() + "'", null);
                if (cek.getCount() > 0) {
                    Cursor c = db.rawQuery("SELECT * FROM ISIBARANG WHERE KDBAR ='" + kdbar.getText().toString() + "'", null);
                    c.moveToFirst();
                    kdbar.setText("");
                    nambar.setText("");
                    satbar.setText("");
                    ...
                    hrgbar.setText(c.getString(c.getColumnIndex("HRGJUAL")));
                    Toast.makeText(getApplicationContext(), "Data tersedia, dan berhasil diambil!", Toast.LENGTH_SHORT).show();
                } else {
...
//                harga = Integer.parseInt(hrgbar.getText().toString());
//                jumlah = Integer.parseInt(jml.getText().toString());
//                total = harga*jumlah;
//                tot.setText(String.valueOf(total));

                //Cursor cek = db.rawQuery("SELECT * FROM TRANSBELI WHERE KDBELI ='" + kdtrans.getText().toString() + "'", null);
                Cursor cekk = db.rawQuery("SELECT * FROM DETAILJUAL1 WHERE KDBARANG ='" + kdbar.getText().toString() + "' AND KDJUAL ='" + kdtrans.getText().toString() + "'", null);
                if (cekk.getCount()>0) {
                    //db.execSQL("UPDATE TRANSBELI SET NAMASUP = '" + sup.getText().toString() + "', TGLBELI = '" + dateView.getText().toString()+ "' WHERE KDBELI ='" + kdtrans.getText().toString() + "';");
                    db.execSQL("UPDATE DETAILJUAL1 SET KDBARANG = '"
                            + kdbar.getText().toString() + "', NAMABRG = '"
                            + nambar.getText().toString() + "', SATUAN = '"
                            + satbar.getText().toString() + "', HARGA = '"
                            + hrgbar.getText() + "', JUMLAH = '"
                            + jml.getText() + "' WHERE KDJUAL ='"
                            + kdtrans.getText().toString() + "';");
                    Toast.makeText(ActivityTransJual_Proto.this, "Data Berhasil Diubah", Toast.LENGTH_LONG).show();
                } else {
                    //db.execSQL("INSERT INTO TRANSBELI VALUES('" + kdtrans.getText().toString() +"','" + sup.getText().toString() + "','" + dateView.getText().toString()+ "';");
                    db.execSQL("INSERT INTO DETAILJUAL1  VALUES('"+ kdtrans.getText().toString() +"','"
                    ...
                    Toast.makeText(ActivityTransJual_Proto.this, "Data Berhasil Ditambah", Toast.LENGTH_LONG).show();
                }
                kdbar.setText("");
                nambar.setText("");
                satbar.setText("");
        ...
        cek.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(ActivityTransJual_Proto.this, ActivityTampilTransJual.class);
                Cursor tot = db.rawQuery("SELECT SUM(HARGA*JUMLAH) AS TOTAL FROM DETAILJUAL1 WHERE KDJUAL ='" + kdtrans.getText().toString() + "'",null);
                tot.moveToFirst();
